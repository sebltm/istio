...
SHA=$(grep "sebltm/api" go.mod | grep "^replace" | awk -F "-" '{print $NF}')
if [ -n "${SHA}" ]; then
  REPO=$(grep "sebltm/api" go.mod | grep "^replace" | awk '{print $4}')
else
  SHA=$(grep "sebltm/api" go.mod | head -n1 | awk -F "-" '{print $NF}')
fi
